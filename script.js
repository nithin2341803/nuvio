...
const contactModal=$('#contactModal'),builderModal=$('#builderModal'),builderForm=$('#builderForm');
...
function fillProject(p){if(!builderForm)return;Object.keys(p||{}).forEach(k=>{const el=builderForm.elements[k];if(!el)return;if(el.type==='radio'){const r=[...builderForm.querySelectorAll(`[name="${k}"]`)].find(x=>x.value===p[k]);if(r)r.checked=true}else el.value=p[k]||''});show(1);updatePreview()}
function openProjectFromUrl(){const slug=new URLSearchParams(location.search).get('project');if(!slug)return;const raw=localStorage.getItem('nuvio-starter-'+slug);if(raw){try{open(builderModal);fillProject(JSON.parse(raw))}catch(e){}}}
$$('.next').forEach(b=>b.addEventListener('click',()=>{if(step===1&&!builderForm?.elements.businessName.value.trim()){builderForm.elements.businessName.focus();return}show(Math.min(4,step+1))}));
...
function updatePreview(){if(!builderForm)return;const f=new FormData(builderForm),name=f.get('businessName')||'YOUR BUSINESS',theme=f.get('theme')||'blue',p=$('#sitePreview');if(!p)return;p.className='site-preview '+theme+'-theme';p.querySelector('small').textContent=(name+' / NUVIO').toUpperCase();p.querySelector('h3').textContent=name.toUpperCase();p.querySelector('p').textContent=f.get('description')||f.get('tagline')||'Professional, clear and ready for customers.';const bar=$('.browser-bar span');if(bar)bar.textContent=slug(name)+'.nuvio';const url=$('#urlPreview');if(url)url.textContent=slug(name)}
builderForm?.addEventListener('input',updatePreview);builderForm?.addEventListener('change',updatePreview);
builderForm?.addEventListener('submit',e=>{e.preventDefault();const d=Object.fromEntries(new FormData(e.target)),s=slug(d.businessName);const project={...d,slug:s,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),status:'Draft'};localStorage.setItem('nuvio-starter-'+s,JSON.stringify(project));const index=JSON.parse(localStorage.getItem('nuvio-projects')||'[]').filter(x=>x.slug!==s);index.unshift({slug:s,name:d.businessName,createdAt:project.createdAt,updatedAt:project.updatedAt,status:'Draft'});localStorage.setItem('nuvio-projects',JSON.stringify(index));alert(`Project saved.\n\nOpen My Projects to continue editing it.\n\nDraft address: ${s}.nuvio`);close(builderModal)});
...
updatePreview();openProjectFromUrl();
})();