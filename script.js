(()=>{
  ...
  // Safety fallback: the homepage must never disappear just because an animation library fails to load.
  const safety=document.createElement('style');
  safety.textContent='.reveal{opacity:1!important;transform:none!important}.hero-copy,.hero-product{opacity:1!important}';
  ...
  // Keep one clear My Projects action instead of adding duplicates to the header.
  const nav=$('.nav nav');
  const navActions=$('.nav-actions');
  if(nav && !nav.querySelector('a[href="starter-projects.html"],a[href="projects.html"]')){
    const a=document.createElement('a');
    a.href='starter-projects.html'; a.textContent='My Projects'; nav.appendChild(a);
  }
  if(navActions){
    const projectLinks=navActions.querySelectorAll('a[href="starter-projects.html"],a[href="projects.html"]');
    projectLinks.forEach((a,i)=>{if(i>0)a.remove()});
    if(!navActions.querySelector('a[href="starter-projects.html"],a[href="projects.html"]')){
      const a=document.createElement('a'); a.href='starter-projects.html'; a.className='ghost'; a.textContent='My Projects'; navActions.insertBefore(a,navActions.firstChild);
    }
  }
  ...
  window.addEventListener('pointermove',e=>{
    if(!glow)return;
    const x=(e.clientX/innerWidth-.5)*24,y=(e.clientY/innerHeight-.5)*18;
